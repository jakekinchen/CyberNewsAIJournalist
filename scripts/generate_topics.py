...
def generate_topics(supabase, amount_of_topics, gpt_ordering = False):
    ...
    if gpt_ordering:  # Only attempt to order if gpt_ordering is True
        try:
            order_topics(topics, amount_of_topics)
        except Exception as e:
            print(f'Failed to order topics, inserting without order: {e}')
    
    top_topics = topics[:amount_of_topics] 
...
def order_topics(topics, amount_of_topics):
